...
catos_err_t catos_kernel_init(void)
{
    ...
    /* 2. 初始化任务池与就绪队列 */
    ktask_init_pool();
    catos_ready_q_init(&core->ready);
    core->current = NULL;

    /* 3. 创建空闲任务：最低优先级，恒在就绪队列。
     *    其底层执行体（线程）由调用 catos_kernel_start 的线程扮演。 */
    k_idle = ktask_alloc("idle", CATOS_CFG_NUM_PRIO - 1,
                         CATOS_CFG_DEFAULT_STACK, k_idle_task, core);
    if (k_idle == NULL)
        return CATOS_E_NOMEM;
    catos_ready_q_add(&core->ready, k_idle);
...
void catos_sched_yield(void)
{
    ...
    if (core->current != NULL && core->current->state == CATOS_TASK_RUNNING) {
        /* 移到同优先级队列尾部，让同优先级任务轮转；更高优先级自然被选出 */
        catos_ready_q_remove(&core->ready, core->current);
        catos_ready_q_add(&core->ready, core->current);
        if (g_sched_started)
            k_sched_from_task();
    }
    catos_port_critical_exit();
